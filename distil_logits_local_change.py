...
config = {
    ...
    "dataset": {
        ...
        "local_file_path": "/mnt/Data/projects/DistillKit/data/20240816_distrill_data_01.json",
        # 使用Hugging face在线文件
...
dataset = load_dataset(config["dataset"]["name"], split=config["dataset"]["split"]) \
    if not config["dataset"]["use_local_files"] else \
    load_dataset(config["dataset"]["local_file_type"], split=config["dataset"]["split"],
                 data_files=config["dataset"]["local_file_path"])
